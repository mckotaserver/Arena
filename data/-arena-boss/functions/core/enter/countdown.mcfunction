...
# 通知
execute as @a[tag=Arena.Player] if score @s Arena matches 101.. run tellraw @s [{"translate":"kota-server.arena.game.message.prefix"},{"translate":"kota-server.arena.boss.game.message.start_countdown","with":[{"nbt":"StartCountdown","storage":"arena-boss:temp","color": "yellow","bold": true}]}]
