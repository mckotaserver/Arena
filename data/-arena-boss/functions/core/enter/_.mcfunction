## 入場時
# トリガー解除
advancement revoke @s only arena-boss:stage_entering/left/confirm
advancement revoke @s only arena-boss:stage_entering/right/confirm
...
# 入場拒否
    ...
    execute store result score $MaxPlayerCount ArenaBoss.Temp run data get storage arena:temp Boss.StageInfo[0].MaxPlayerCount
...
data modify storage arena-boss: Name set from storage arena:temp Boss.StageInfo[0].MetaData.Name
