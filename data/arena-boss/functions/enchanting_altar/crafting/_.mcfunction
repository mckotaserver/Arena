...
## そもそもクラフトできるか確認
    ...
    # エラー出力
    execute if data storage arena-boss:temp {Altar:{HasError:true}} run tellraw @a[tag=Arena.AltarOpener] [{"translate":"kota-server.arena.game.message.prefix"},{"translate":"kota-server.arena.boss.enchantment_altar.message.error.invalid_recipe"}]
...
## 既にエンチャントが付いている場合、違うエンチャントを付けようとしていないか確認
    ...
    # エラー出力
    execute if data storage arena-boss:temp {Altar:{Operation:Enhance}} if data storage arena-boss:temp {Altar:{HasError:true}} run tellraw @a[tag=Arena.AltarOpener] [{"translate":"kota-server.arena.game.message.prefix"},{"translate":"kota-server.arena.boss.enchantment_altar.message.error.cannot_put_different_enchantments"}]
...
execute if data storage arena-boss:temp {Altar:{EnchantmentLevel:7s}} run tellraw @a[tag=Arena.AltarOpener] [{"translate":"kota-server.arena.game.message.prefix"},{"translate":"kota-server.arena.boss.enchantment_altar.message.error.already_reached_max_level","with":[{"text": "7","color": "red","italic": false}]}]
...
execute store result score $EnchantmentLevelBefore ArenaBoss.Temp run data get storage arena-boss:temp Altar.ResourceItem[0].tag.KotaItems.BossEnchantment.lvl
    ...
    execute if score $E.ShardRequired ArenaBoss.Temp > $E.ShardPlaced ArenaBoss.Temp run tellraw @a[tag=Arena.AltarOpener] [{"translate":"kota-server.arena.game.message.prefix"},{"translate":"kota-server.arena.boss.enchantment_altar.message.error.not_enough_resource","with":[{"translate": "arena.item.name.enchantment_shard","color": "red","bold": true,"underlined": true},{"score":{"name": "$E.ShardShortage","objective": "ArenaBoss.Temp"},"color": "red","bold": true,"underlined": true}]}]
...
## 煌めく小片の個数が足りているか確認
    ...
    execute if score $G.ShardRequired ArenaBoss.Temp > $G.ShardPlaced ArenaBoss.Temp run tellraw @a[tag=Arena.AltarOpener] [{"translate":"kota-server.arena.game.message.prefix"},{"translate":"kota-server.arena.boss.enchantment_altar.message.error.not_enough_resource","with":[{"translate": "arena.item.name.glitter_shard","color": "red","bold": true,"underlined": true},{"score":{"name": "$G.ShardShortage","objective": "ArenaBoss.Temp"},"color": "red","bold": true,"underlined": true}]}]
