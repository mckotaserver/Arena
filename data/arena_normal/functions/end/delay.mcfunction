...
execute as @e[tag=Arena.Normal-Stage.Stage-Core] if score #CurrentTick Arena.Temp <= @s Arena.Temp if score @s Arena.Temp <= #CurrentTick-Max Arena.Temp at @s run function arena_normal:end/main with entity @s data.Arena
