## 01-zombie のステージ情報設定
# 項の作成
data modify storage arena:assets stage_difficulty append value {id:"03-lunatic"}

#> その他ステージ情報
# 表示情報
    # 表示名
    data modify storage arena:assets stage_difficulty[-1].display.name set value '{"text":"ルナティック","color":"light_purple","bold":true}'

    # 表示名(翻訳キー)
    data modify storage arena:assets stage_difficulty[-1].display.translation_key set value "kota-server.arena.game.difficulty.lunatic"
