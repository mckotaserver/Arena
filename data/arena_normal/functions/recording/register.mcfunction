...
#> 各データをまとめ
# 表示データ
data modify storage arena_normal:temp recording.Compound.display_data set from storage arena_normal:temp recording.Time.display_data
...
# データベースに挿入
    ...
    data modify storage arena_normal:temp recording.Insertion.name set from entity @e[tag=arena.normal_stage.stage_core,sort=nearest,limit=1] data.arena.Spawning.Detail.name
