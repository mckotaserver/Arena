...
execute store success storage arena:temp Recording.Insertion.Execute byte 1 if score #Recording.DupedUUID-New Arena.Temp < #Recording.DupedUUID-Old Arena.Temp
    ...
    # エンドレスなら結果を反転
    execute if data storage arena:temp {Recording:{Insertion:{name:"エンドレス"}}} unless score #Recording.DupedUUID-New Arena.Temp = #Recording.DupedUUID-Old Arena.Temp store result storage arena:temp Recording.Insertion.Execute byte 1 if data storage arena:temp {Recording:{Insertion:{Execute:false}}}
