...
# 指定のエンティティを消去
kill @e[type=#arena_normal:resetting_entity,distance=..32]
...
execute if data storage arena_normal:temp {marker_existance:false} run kill @e[tag=arena.normal_stage.spawn_marker,distance=..32]
...
execute as @e[tag=arena.normal_stage.spawn_marker,distance=..32] at @s run tp @s ~ ~ ~ facing entity @e[tag=arena.normal_stage.stage_core,sort=nearest,limit=1]
