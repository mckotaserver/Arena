# APあるか判定
    # AP不足
    execute unless score @s arena matches 1.. run tellraw @s [{"translate":"kota-server.arena.game.message.prefix"},{"translate":"kota-server.arena.game.message.error.not_enough_arena_point"}]
...
scoreboard players operation @a[tag=Arena.Player,scores={Arena=0}] Arena = @e[tag=Arena.Entrance,sort=nearest,limit=1] Arena
    ...
    execute as @e[tag=Arena.Core] if score @s Arena = @e[tag=Arena.Entrance,sort=nearest,limit=1] Arena at @s if data entity @s {data:{Arena:{StageType:Normal}}} run tellraw @a[tag=Arena.Player,distance=..20] [{"translate":"kota-server.arena.game.message.prefix"},{"translate":"kota-server.arena.game.message.how_to_change_modes"}]
