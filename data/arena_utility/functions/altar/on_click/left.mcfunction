...
#> エラー出力
    ...
    # アイテムおいてない
    execute unless data storage arena_utility:temp Altar.data.PlacedItem[0] run tellraw @s [{"translate":"kota-server.arena.game.message.prefix"}," ",{"translate":"kota-server.arena.boss.altar.message.error.no_extractable_item"}]
    execute unless data storage arena_utility:temp Altar.data.PlacedItem[0] run playsound entity.experience_orb.pickup master @s ~ ~ ~ 1 0.5
