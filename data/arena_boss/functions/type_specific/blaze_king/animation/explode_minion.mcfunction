...
execute if score @p[tag=Arena.Boss.Core-Player] ArenaBoss.Timer matches 20 at @e[tag=Arena.Boss.Main.Extra-02,distance=..64] if entity @a[tag=Arena.Boss.Player,distance=..4] as @a[tag=Arena.Boss.Player,distance=..4] run damage @s 40 arena_boss:bypasses_cooldown by @e[tag=Arena.Boss.Main.Extra-02,sort=nearest,limit=1]
